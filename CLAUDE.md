...
- **Deux moteurs locaux** : Parakeet (**défaut**, ~5× plus rapide sur les tampons courts, mais `glossary` sans effet) et Whisper (`stt_provider = "local"`, plus lent, glossaire actif). Le moteur par défaut est une **dépendance dure** sur macOS, pas un extra — sinon un `uv sync` nu retombe en silence sur Whisper. Ne jamais proposer `large-v3-turbo` comme accélération : mesuré 2 à 2,7× **plus lent** que `medium` sur ce pipeline. Détails et chiffres dans `benji/stt/CLAUDE.md`.

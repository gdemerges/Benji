...
- **Deux moteurs locaux** : Whisper (défaut, glossaire actif) et Parakeet (`stt_provider = "parakeet"`, `uv sync --extra parakeet`) — ~5× plus rapide sur les tampons courts, mais sans glossaire. Ne jamais proposer `large-v3-turbo` comme accélération : mesuré 2 à 2,7× **plus lent** que `medium` sur ce pipeline. Détails et chiffres dans `benji/stt/CLAUDE.md`.

...
`hotkeys.py` — raccourci **global**, une implémentation par OS derrière `build_hotkeys()` : `GlobalHotkeys` (Carbon, macOS — voir la note du CLAUDE.md racine sur le choix de Carbon plutôt que `NSEvent.addGlobalMonitor`), `WindowsHotkeys` (`RegisterHotKey` + `WM_HOTKEY` intercepté sur la boucle Qt), `LinuxHotkeys` (`XGrabKey`, X11 seulement — échoue silencieusement sous Wayland). `_tokenize()` est pure et partagée par les trois (« Ctrl+Alt+Cmd+B » → modificateurs/touche) ; chaque OS encode ensuite dans ses propres codes via `parse_shortcut()`/`parse_shortcut_windows()`/`parse_shortcut_x11()`, qui refusent tous une touche sans modificateur — la réserver la retirerait de toutes les autres apps. Windows/Linux non validés sur machine réelle.
